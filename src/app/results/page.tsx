...
const ALL_PROGRAMS = Array.from(new Set((localCutoffs as any[]).map(d => d.program))).filter(Boolean).sort() as string[];
...
function ResultsContent() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetchCutoffsForPrediction(initCat, { round: initRound, quota: initQuota, pwd: initPwd, seatType: initSeat })
      .then(data => {
        setResults(sortPredictions(data.map(d => calculatePrediction(initRank, d))));
        setLoading(false);
      });
  }, [initRank, initCat, initPwd, initQuota, initRound, initSeat]);
  ...
  const handleExportPDF = () => {
    generatePredictionPDF(filtered, { 
      ...
      category: initCat, 
      quota: initQuota, 
      tfwStatus: initCat === 'GENERAL_TFW' ? 'Yes' : 'No',
      name: user?.name,
      pwdStatus: initPwd
    });
  ...
  const FilterPanel = (
    <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border-solid)', borderRadius: 16, overflow: 'hidden', height: 'calc(100vh - 100px)', display: 'flex', flexDirection: 'column' }}>
      {/* Header */}
      ...
      {/* Scrollable body */}
      <div style={{ padding: '10px 16px', overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        {categoriesWithPwd.includes(lCat) && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0' }}>
            <input
              ...
              id="pwdSidebarCheck"
              checked={lPwd}
              onChange={(e) => setLPwd(e.target.checked)}
              style={{ width: 16, height: 16, cursor: 'pointer' }}
            />
            <label htmlFor="pwdSidebarCheck" style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', cursor: 'pointer', userSelect: 'none' }}>
